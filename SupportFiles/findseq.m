function varargout = findseq(varargin)

% FINDSEQ Find sequences of repeated (adjacent/consecutive) numeric values
% WARNING: The outputs aren't in the right order when 'dim' ~= 1
%
%   FINDSEQ(A) Find sequences of repeated numeric values in A along the
%              first non-singleton dimension. A should be numeric.
%
%   FINDSEQ(...,'dim', DIM) Look for sequences along the dimension specified by the
%                    positive integer scalar DIM.
%
%   FINDSEQ(...,'findValue', value) Look for sequences of the specifed value only
%
%   FINDSEQ(...,'minLength', length) Look for sequences as long as or
...
% Edited by Abbey Roelofs, aheinlei@umich.edu
% 19 jan 2018 - Adding input options to search for a specific value and minimum sequence length
...
addParameter(p, 'dim', []);
addParameter(p, 'findValue', [], @isnumeric);
addParameter(p, 'minLength', [], @isnumeric);
parse(p, varargin{:});
A = p.Results.A;
dim = p.Results.dim;
findValue = p.Results.findValue;
minLength = p.Results.minLength;

% IN
if ~isnumeric(A) && ~islogical(A)
    error('findseq:fmtA', 'A should be numeric or logical')
...
% If a value to find was specified, get rid of all the out values that
% aren't this value.
if ~isempty(findValue)
    Out = Out(Out(:,1)==findValue, :);
end
    
% If we only want sequences as long as or longer than a minimum length, get
% rid of any that don't meet this requirement.
if ~isempty(minLength)
    Out = Out(Out(:,4)>=minLength, :);
end
